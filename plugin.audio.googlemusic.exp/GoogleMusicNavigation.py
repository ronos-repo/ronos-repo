...
class GoogleMusicNavigation():
    ...
    def executeAction(self, params):
        ...
        if (action == "play_all"):
            ...
            self.main.log("TEST add library "+repr(params))
            self.api.addAAtrack(params["song_id"])
            self.api.clearCache()
        elif (action == "add_playlist"):
            self.main.log("TEST add playlist "+repr(params))
            self.add2Playlist(params["song_id"])
        elif (action == "del_from_playlist"):
            self.main.log("TEST dell from playlist "+repr(params))
    ...
    def add2Playlist (self, song_id):
        ...
        if selected > 0:
            ...
            self.api.add2Playlist(playlist_id, song_id)
