...
def ShowRadioList(plugin, token):
	...
	xbmcplugin.setContent(handle, 'Radio')
	xbmcplugin.endOfDirectory(handle,True,False)
	xbmc.executebuiltin("Container.SetViewMode(500)")
...
def EpgDateList(plugin,params,token):
	...
	for channel in channels['data']['epg']:
		...
		channel_information = '%s:%s - %s:%s         [B]%s[/B] \n %s' % (channel['begin'][8:10],channel['begin'][10:12],channel['end'][8:10],channel['end'][10:12],channel['title'], channel['description'])
	...
	xbmcplugin.setContent(handle, 'Recorded')
	xbmcplugin.endOfDirectory(handle,True,False)
	xbmc.executebuiltin("Container.SetViewMode(51)")
