...
def add_item( itemcount=1,action="" , title="" , plot="" , url="" ,thumbnail="" , extra="",mmsId="",page="",token="", isPlayable = False, folder=True ):
    listitem = xbmcgui.ListItem( title, iconImage="DefaultVideo.png", thumbnailImage=thumbnail )
    listitem.setInfo( "video", { "Title" : title, "FileName" : title, "Plot" : plot } )
    commands = []
    commands.append(( u'פרטים', 'XBMC.Action(Info)'))

    listitem.addContextMenuItems( commands )
    if url.startswith("plugin://"):
        itemurl = url
        listitem.setProperty('IsPlayable', 'true')
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder, totalItems=itemcount)
    elif isPlayable:
        listitem.setProperty("Video", "true")
        listitem.setProperty('IsPlayable', 'true')
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&extra=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title ) , urllib.quote_plus(url) , urllib.quote_plus( thumbnail ) , urllib.quote_plus( extra ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder , totalItems=itemcount)
    else:
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&extra=%s&page=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title.encode('utf-8') ) , urllib.quote_plus(url.encode('utf-8')) , urllib.quote_plus( thumbnail.encode('utf-8') ) , urllib.quote_plus( extra.encode('utf-8') ), urllib.quote_plus(page ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder,totalItems=itemcount)
